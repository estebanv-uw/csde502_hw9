...
s.df %<>% cbind(hh_mtotal = p.df$hh_mtotal)
...
Similar to the process of counting the total number of household members, we create a proxy data set counting the relationship of each household member to the respondent, and then summate the total responses of "sister"
...
p.df %<>% mutate(hh_stotal = rowSums(p.df) + 1)

#Step 3.0.4: Binding results to main data frame
s.df %<>% cbind(hh_stotal = p.df$hh_stotal)
